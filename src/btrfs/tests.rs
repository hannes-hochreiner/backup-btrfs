...
fn get_subvolume_info_1() {
    ...
    let mut btrfs = Btrfs {
        command: Box::new(CommandMock {
            commands: vec![(
                String::from("sudo btrfs subvolume show \"/data\""),
                ctx.clone(),
            )],
            responses: vec![String::from(
                r"/
    ...
    assert_eq!(
        btrfs.get_subvolume_info("/data", &ctx).unwrap(),
        SubvolumeInfo {
...
fn get_subvolume_info_2() {
    ...
    let mut btrfs = Btrfs {
        command: Box::new(CommandMock {
            commands: vec![(
                String::from("sudo btrfs subvolume show \"/home\""),
                ctx.clone(),
            )],
            responses: vec![String::from(
                r"home
    ...
    assert_eq!(
        btrfs.get_subvolume_info("/home", &ctx).unwrap(),
        SubvolumeInfo {
